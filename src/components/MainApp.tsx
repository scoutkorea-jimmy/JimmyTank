...
export default function MainApp() {
  ...
  const { allPersonas, getPersona } = usePersona();
  const { auth, logout } = useAuth();

  // UI State
  const [activeTab, setActiveTab] = useState<"tanks" | "discussion">("tanks");
  ...
  const [selectedMembers, setSelectedMembers] = useState<string[]>(["marketer", "finance", "tech"]);
  ...
  return (
    <div className="flex h-screen flex-col">
                ...
                {state.tanks.sort((a, b) => b.updatedAt - a.updatedAt).map((tank) => (
                  ...
                  >
                    <div className="mb-1 flex items-center gap-1.5">
                      <Badge variant={tank.engine === "chatgpt" ? "green" : "orange"}>
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); handleDelete(tank.id); }}
                        className="ml-auto rounded p-0.5 text-zinc-300 opacity-0 hover:text-red-500 group-hover:opacity-100"
                      >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-xs text-zinc-400">
                      {activeTank.members.map((id) => getPersona(id)?.name || "?").join(", ")}
                    </span>
                  </div>
                </div>
                {activeTank.summary && (
                  <div className="hidden max-w-xs rounded-lg bg-blue-50 px-3 py-2 text-xs text-blue-700 lg:block dark:bg-blue-950 dark:text-blue-300">
                    <span className="font-semibold">요약:</span> {activeTank.summary.slice(0, 100)}...
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {activeTank.messages.length === 0 ? (
                  <div className="flex h-full flex-col items-center justify-center">
                    ...
                    <Button onClick={startDiscussion} disabled={isLoading}>
                      {isLoading ? "시작 중..." : "🚀 토론 시작"}
                    </Button>
                  </div>
                ) : (
              ...
              {activeTank.messages.length > 0 && (
                <div className="border-t border-zinc-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-950">
                  <div className="mx-auto flex max-w-3xl items-center gap-2">
                    ...
                    />
                    <Button onClick={sendUserMessage} disabled={isLoading || !input.trim()} className="rounded-full">
                      전송
                    </Button>
                    <Button variant="secondary" onClick={continueDiscussion} disabled={isLoading} className="rounded-full" title="추가 토론">
                      🔄
                    </Button>
                  </div>
                </div>
      ...
      <footer className="flex h-10 items-center justify-between border-t border-zinc-200 bg-white px-4 text-xs text-zinc-400 dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <button onClick={() => setShowVersionHistory(true)} className="hover:text-blue-500 transition-colors">
          Version History
        </button>
      </footer>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => setEngine("claude")}
                className={`flex flex-1 items-center justify-center gap-2 rounded-xl border-2 px-4 py-3 text-sm font-medium transition-all ${engine === "claude" ? "border-orange-500 bg-orange-50 text-orange-700 dark:border-orange-400 dark:bg-orange-950 dark:text-orange-300" : "border-zinc-200 text-zinc-600 dark:border-zinc-700"}`}>
                🟠 Claude
              </button>
              <button type="button" onClick={() => setEngine("chatgpt")}
                className={`flex flex-1 items-center justify-center gap-2 rounded-xl border-2 px-4 py-3 text-sm font-medium transition-all ${engine === "chatgpt" ? "border-green-500 bg-green-50 text-green-700 dark:border-green-400 dark:bg-green-950 dark:text-green-300" : "border-zinc-200 text-zinc-600 dark:border-zinc-700"}`}>
                🟢 ChatGPT
              </button>
            </div>
          </div>
